...
export default function GiftCoinsModal({ open, onClose, recipientUsername, recipientAvatar, recipientFullname, postId }) {
  ...
  return (
    <>
      ...
      <div
        className={cn(
          'fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-900 rounded-t-3xl z-50 flex flex-col',
          'md:inset-0 md:m-auto md:rounded-2xl md:max-w-sm md:h-fit md:border md:border-gray-100 dark:md:border-gray-800 md:shadow-2xl'
        )}
      >
        <div className="md:hidden flex justify-center pt-2.5 pb-1">
          <div className="w-8 h-1 rounded-full bg-gray-200 dark:bg-gray-800" />
        </div>

        {/* Header */}
        <div className="flex items-center justify-between px-5 pt-3 pb-3 border-b border-gray-100 dark:border-gray-800">
          <h3 className="text-sm font-black text-gray-900 dark:text-gray-100">Gift Mobcoins</h3>
          <button
            onClick={onClose}
        ...
        <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-50 dark:border-gray-800/60 bg-gray-50/50 dark:bg-gray-900/20">
          ...
          <div className="flex-1">
            <p className="text-xs font-bold text-gray-900 dark:text-gray-100">{recipientFullname || recipientUsername}</p>
            <p className="text-[11px] font-mono text-gray-400 mt-0.5">@{recipientUsername}</p>
          </div>
          {balance !== null && (
            <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-full bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-900/30">
              ...
              <span className="text-[11px] font-bold text-amber-600 dark:text-amber-400">{balance}</span>
            </div>
          )}
        </div>
        ...
        <div className="px-5 pt-3 pb-2">
          ...
          <div className="flex items-center gap-2.5 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500 transition-all bg-transparent">
            ...
            <input
              ...
              className="flex-1 text-sm bg-transparent outline-none text-gray-900 dark:text-gray-100 placeholder-gray-300 dark:placeholder-gray-600 font-bold"
              autoFocus
            />
            <span className="text-xs font-bold text-gray-400 dark:text-gray-500">Mobcoins</span>
          </div>
        </div>

        {/* Status messages */}
        {status?.error && (
          <div className="mx-5 mt-2 mb-1 text-xs font-semibold text-red-600 bg-red-50 dark:bg-red-950/20 border border-red-100 dark:border-red-900/20 rounded-xl px-3.5 py-2.5">
            {status.error}
          </div>
        )}
        {status?.success && (
          <div className="mx-5 mt-2 mb-1 text-xs font-semibold text-green-700 bg-green-50 dark:bg-green-950/20 border border-green-100 dark:border-green-900/20 rounded-xl px-3.5 py-2.5">
            {status.success}
          </div>
        )}

        {/* Actions */}
        <div className="px-5 pb-8 md:pb-4 pt-4 flex gap-2 border-t border-gray-50 dark:border-gray-800 mt-3">
          <button
            onClick={onClose}
